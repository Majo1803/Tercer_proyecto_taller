...
#**************Listar actividades******************
    def listar(self):
        ...
        while aux !=None:
            print(aux)
            aux = aux.siguiente 
            actividades_txt=open("actividades.txt","wb")
            lista=list(lista_actividades_disponibles)
            pickle.dump(lista_actividades_disponibles,actividades_txt)
            actividades_txt.close() 
            actividades_imprimir=open("actividades.txt","rb")
            actividades_texto=pickle.load(actividades_imprimir)
            print(actividades_texto) 
        return None           
    ...
    def iniciar_act(self, nombre,cont_analizador,cont):
            ...
            while aux!=None:
                if aux.nombre == nombre: #si el nombre recibido es igual al de una actividad registrada
                    ...
                    distraccion=schedule.every(30).seconds.do(analizador_distracciones_imagen,cont_d,cont_i,cont_b,cont_a,final,cont_analizador)
                    ...
                    while True:
                        if aux.f_h_inicio<=ahora: #si la fecha y hora de inicio de la actividad es mayor a ahora
                            ...
                            #distraccion_hilo=threading.Thread(target=analizador_distracciones_imagen,args=(cont_d,cont_i,cont_b,cont_a,final))
...
def inicio_actividades():
    ...
    while(True):
        print("\n"+
              ...
              "2. Guardar y listar actividades existentes\n"+
        ...
        if num == "1":
            ...
            try:  
                with open ("actividades.txt",errors='ignore') as f:
                ...
                print("El archivo para validar carreras aún no ha sido creado, o no se ha guardado ningún registro") 
...
class rostro_d ():
    ...
    def capturar_imagen(self,vista,cuenta_regresiva_dis):
        # toma la foto en 30 segundos
        if cuenta_regresiva_dis:
            cont=5
            while cont>0:
...
def analizador_distracciones_imagen(cont_d,cont_i,cont_b,cont_a,final,cont_analizador):
    cont_analizador=cont_analizador # contador, este es para llevar una cuenta de los primeros 5 minutos segun las fotos tomadas (5 minutos=5 fotos)
    toma_distraccion=rostro_d()
    imagen=toma_distraccion.capturar_imagen(vista=False,cuenta_regresiva_dis=True)
    ...
    if len(faces)>1:
       messagebox.showwarning(title='Alerta',message='Hemos detectado más de un rostro, la fotografía volverá a tomarse')
       dis=schedule.every(5).seconds.do(analizador_distracciones_imagen(cont_d,cont_i,cont_b,cont_a,final)) 
    ...
    if face_angles['pan_angle'] <-35: # distraccion a la derecha
        ...
        while True:
            if cont_d ==5:
                ...
                sleep(1) 
                cont_d=0
                emo=schedule.every(1).do(analizador_emociones_imagen(cont_analizador,final)) 
                emo.scheduler.run_pending(cont_analizador,final)  
            elif cont_d < 5:
                print("Contando derecha")
                cont_d=cont_d+1
                dis=schedule.every(5).seconds.do(analizador_distracciones_imagen(cont_d,cont_i,cont_b,cont_a,final,cont_analizador))
                dis.scheduler.run_pending(cont_d,cont_i,cont_b,cont_a,final,cont_analizador)   
